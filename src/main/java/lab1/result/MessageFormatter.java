...
public final class MessageFormatter {
    ...
    public static String onePoint(Point p) {
        return "Єдина точка перетину прямих (x0, y0) = (" + fmt(p.x) + ", " + fmt(p.y) + ")";
    }
    ...
    public static String threePoints(Point p1, Point p2, Point p3) {
        return "Три точки перетину прямих "
                + "(x1, y1) = (" + fmt(p1.x) + ", " + fmt(p1.y) + "), "
                + "(x2, y2) = (" + fmt(p2.x) + ", " + fmt(p2.y) + "), "
                + "(x3, y3) = (" + fmt(p3.x) + ", " + fmt(p3.y) + "), i=1,2,3";
    }
